...
| 名称 | 类型 | 说明 |
|---|---|---|
| `Result.Id` | String | 单据内码 |
| `Result.Number` | String | 单据编码 |
| `Result.NeedReturnData` | Array | 返回数据（对应请求 `NeedReturnFields`） |
| `Result.ResponseStatus.IsSuccess` | Boolean | 操作是否成功 |
| `Result.ResponseStatus.ErrorCode` | String | 错误代码，失败时为 `"500"` |
| `Result.ResponseStatus.Errors` | Array | 错误列表 |
| `Result.ResponseStatus.Errors[].FieldName` | String | 字段名称 |
| `Result.ResponseStatus.Errors[].Message` | String | 错误消息 |
| `Result.ResponseStatus.Errors[].DIndex` | Integer | 原始数据行号 |
| `Result.ResponseStatus.SuccessEntities` | Array | 成功列表 |
| `Result.ResponseStatus.SuccessEntities[].Id` | String | 成功的内码 |
| `Result.ResponseStatus.SuccessEntities[].Number` | String | 成功的编码 |
| `Result.ResponseStatus.SuccessEntities[].DIndex` | Integer | 原始数据行号 |
| `Result.ResponseStatus.SuccessMessages` | Array | 提示信息列表 |
| `Result.ResponseStatus.SuccessMessages[].FieldName` | String | 字段名称 |
| `Result.ResponseStatus.SuccessMessages[].Message` | String | 提示内容 |
| `Result.ResponseStatus.SuccessMessages[].DIndex` | Integer | 原始数据行号 |
