            ...
            Context ctx, JmPdfOrder order, List<string> tierNumbers, List<string> componentNumbers)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < order.Tiers.Count; i++)
                {
                    ...
                    view.Model.SetValue("FQTY", 1, row);
                    view.Model.SetValue("FBaseUnitQty", 1, row);    // 销售基本数量 = 销售数量（演示环境基本单位=销售单位，换算 1:1，避免下推生产订单因基本数量为0被拦）
                    view.Model.SetValue("FPRICEUNITQTY", 1, row);   // 计价数量 = 销售数量
                    view.Model.SetValue("FPRICEBASEQTY", 1, row);   // 计价基本数量 = 销售数量
                    view.Model.SetValue("FSTOCKBASEQTY", 1, row);   // 库存基本数量 = 销售数量
                    view.Model.SetValue("FSTOCKQTY", 1, row);   // 库存数量 = 销售数量
