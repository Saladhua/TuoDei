            ...
            Context ctx, JmPdfOrder order, List<string> tierNumbers, List<string> componentNumbers)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < order.Tiers.Count; i++)
                {
                    ...
                    view.Model.SetValue("FPRICEBASEQTY", decimal.Parse("1", System.Globalization.CultureInfo.InvariantCulture), row);   // 计价基本数量 = 销售数量（去0）
                    view.Model.SetValue("FSTOCKBASEQTY", decimal.Parse("1", System.Globalization.CultureInfo.InvariantCulture), row);   // 库存基本数量 = 销售数量（去0）
                    view.Model.SetValue("FSTOCKQTY", 1m, row);   // 库存数量 = 销售数量
